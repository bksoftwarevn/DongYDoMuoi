    ...
    if(rootPage !== 'null') {
       if(rootPage == 1) {
           ...
           productTypeFindById(idProductType).then(rs => {
               if(rs) {
                   viewInforCategoryPage(rs.image, rs.name, rs.description, rs.attachment);
               }
           }).catch(err => {
               console.log(err);
...
function viewInforCategoryPage(img, name, desc, catalog) {
    ...
    $("#file-catalog").attr("href", viewSrcFile(catalog));
}
